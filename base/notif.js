					...
					$.parseHTML(`
						<li>
							<div class="notification" id="notif_${uid}">
								...
								<span class="notifmsg"><a href="${$(this).find(`a.user`).attr("href")}">~${$(this).find(`a.user`).text()}</a> reviewed <a href="${$(this).find(`a.album`).attr("href")}">${$(this).find(`a.album`).text()}</span>
							</div>
						</li>
					`)
				);
				...
				$("#notifications ul.dropdown-menu").append(
					$("<li/>").append(
						$("<div/>", {
							...
							$("<span/>", {
								class: "notifmsg"
							}).html(`<a href="${$(this).find(`a.user`).attr("href")}">~${$(this).find(`a.user`).text()}</a> reviewed ${$(this).find(`a.album`).text()}`)
						)
					)
				);
				$(`div#notif_${uid}`).click(() => {
